...
export function wireEvents(managerApi, managerState) {
    ...
    // Scheduling (requirements spec 1.32): its own field class (not
    // .se-indy-field) because it saves through updateIndependentPresetSchedule
    // (its own merge/validation/nextRun computation), not
    // updateIndependentPreset's generic config patch. One field, one
    // on-change save, then a full re-render - same convention as
    // .se-indy-field above; a rejected save (bad mode/value) writes nothing,
    // so the re-render reverts the field to its last stored value.
    $overlay.on('change', '.se-indy-schedule-field', function () {
        const presetId = $(this).attr('data-preset-id');
        const field = $(this).attr('data-field');
        const preset = managerApi.getSettings().presets[presetId];
        if (!preset) return;
        ...
        const result = managerApi.updateIndependentPresetSchedule(presetId, { [field]: value });
        uiRender.renderPresetsTab(managerApi, managerState.currentPresetId, managerState.presetsSubtab);
        managerApi.setStatus(result !== null
            ? `Updated "${preset.name}"'s schedule.`
            : `Could not update "${preset.name}"'s schedule - check the mode, value and calendar (see the console for the exact reason).`, result === null);
    });
    ...
    $overlay.on('click', '.se-manager-save-variable-inline', function () {
        ...
        // This editor writes preset.variables directly (below) rather than
        // going through createVariable()/updateVariable(), so
        // checkedDatetime()'s rules (variable-api.js) never run for it -
        // re-checked here instead, same reason the calendar/defaultValue
        // checks just above are duplicated rather than shared. A self-
        // reference is not checked: the dropdown (otherVars) already
        // excludes the variable being edited, so it can never be offered.
        // (An earlier version of this feature also validated a separate
        // tickUnit field here; removed 2026-09-22 along with fixedIncrement/
        // accumulate - see variable-schema.js.)
